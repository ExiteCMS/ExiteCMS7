	...
	if ($message != "") {
		...
		if (isset($_SESSION['posts'][$random_id])) {
			...
			if (!$flood) {
				...
				$result = dbquery("INSERT INTO ".$db_prefix."posts (forum_id, thread_id, post_subject, post_message, post_showsig, post_smileys, post_author, post_datestamp, post_ip, post_cc, post_edituser, post_edittime) VALUES ('$forum_id', '$thread_id', '".mysqli_real_escape_string($_db_link, $subject)."', '".mysqli_real_escape_string($_db_link, $message)."', '$sig', '$smileys', '".$userdata['user_id']."', '".time()."', '".USER_IP."', '".$userdata['user_cc_code']."', '0', '0')");
				$newpost_id = mysqli_insert_id($_db_link);
					...
					if (!$tdata['thread_locked']) {
						...
						if ($userdata['user_id'] == $data['post_author']) {
							if ($data['post_sticky'] == 1) {
								...
								$data['user_can_edit'] = true;
							} elseif ($settings['forum_edit_timeout_on_post'] == 0 && (max($data['post_datestamp'], $data['post_edittime']) + $settings['forum_edit_timeout'] * 60) > time()) {
								// timeout is within the last edit date (or post date)
								$data['user_can_edit'] = true;
							} elseif ($settings['forum_edit_timeout_on_post'] == 1 && ($data['post_datestamp'] + $settings['forum_edit_timeout'] * 60) > time()) {
