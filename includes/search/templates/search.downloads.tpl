...
{if $action == "search"}
	{section name=idx loop=$reportvars.output}
		...
		{if $reportvars.output[idx].download_description}
			&nbsp;&middot;&nbsp;{$reportvars.output[idx].download_description}<br />
		{/if}
		<span class='small'><font class='smallalt'>{$locale.src433}</font> {$reportvars.output[idx].download_license},
		<font class='smallalt'>{$locale.src434}</font> {$reportvars.output[idx].download_os},
		<font class='smallalt'>{$locale.src435}</font> {$reportvars.output[idx].download_version}
		<font class='smallalt'>{$locale.src436}</font>{$reportvars.output[idx].download_datestamp|date_format:"longdate"},
		<font class='smallalt'>{$locale.src437}</font> {$reportvars.output[idx].download_count}</span>
	{/section}
{else}
