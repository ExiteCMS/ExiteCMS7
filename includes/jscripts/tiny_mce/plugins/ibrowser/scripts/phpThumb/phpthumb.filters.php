	...
	function WatermarkText(&$gdimg, $text, $size, $alignment, $hex_color='000000', $ttffont='', $opacity=100, $margin=5, $angle=0, $bg_color=false, $bg_opacity=0, $fillextend='') {
		...
		if (@is_readable($ttffont) && is_file($ttffont)) {
			...
			if ($alignment == '*') {
				...
				$x_TL = strpos($fillextend, 'x') !== false ?               0 : min($x1, $x2);
				$y_TL = strpos($fillextend, 'y') !== false ?               0 : min($y1, $y2);
				$x_BR = strpos($fillextend, 'x') !== false ? ImageSX($gdimg) : max($x1, $x2);
				$y_BR = strpos($fillextend, 'y') !== false ? ImageSY($gdimg) : max($y1, $y2);
