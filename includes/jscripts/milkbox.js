/*
	...
	modifications by WanWizard, ExiteCMS:
	- feature: added open resizeToMaxSize, if true the pictures will be resized to the max size given
	- feature: added function to calculate the max size based on the window size (always-fit-option)
*/
...
var Milkbox = new Class({
	...
	showThisImage:function(image,description){
 	...
 	showImage:function(image){
		...
		if (this.options.resizeToMaxSize) {
	 		var imageSize = new Hash(image.getProperties('width','height')).map(function(item, index){
				return item.toInt();
			});
			var divider1 = 1; var divider2 = 1;
			if (imageSize.height > this.options.max_height) divider1 = this.options.max_height / imageSize.height;
			if (imageSize.width > this.options.max_width) divider2 = this.options.max_width / imageSize.width;
			image.setProperty('height', imageSize.height * Math.min(divider1, divider2));
			image.setProperty('width', imageSize.width * Math.min(divider1, divider2));
	 		imageSize = new Hash(image.getProperties('width','height')).map(function(item, index){
				return item.toInt();
			});
		} else {
	 		var imageSize = new Hash(image.getProperties('width','height')).map(function(item, index){
		...
		this.image.setStyles({'width':targetSize.width, 'height':targetSize.height})
...
//		};
	...
	prepareGalleries:function(){
		...
		$$('a').each(function(a){
			...
			if(a.rel && a.rel.test(/^milkbox/i) && a.href.split('?')[0].test(/\.(gif|jpg|png|img)$/i)){
