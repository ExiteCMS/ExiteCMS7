<?php
	...
	$files = array();	
	if (!is_array($cfg['ilibs_dir'])) $cfg['ilibs_dir'] = array($cfg['ilibs_dir']);
	...
	function dirlist(&$files,$dir) {		
		...
		if ($handle = opendir($dir)) {			
			while ($file = readdir($handle)) {				
				if ($file == '.' || $file == '..' || $file{0} == "." || $file == "avatars" || $file == "flags" || $file == "ranking" || $file == "smiley" || substr($file,-5) == "_cats" || $file == "" || $file == "") {
					continue;					
				}				
