...
var FancyUpload2 = new Class({
	...
	options: {
		...
		removeButton: "X",	// ** EXITECMS MOD ** texts, as options so we can receive translations from the CMS engine
		uploadWith: "X",
		timeLeft: "X",
		uploadComplete: "X",
		overallProgress: "X",
	...
	render: function() {
		...
		// ExiteCMS MOD: get the upload counter element to dynamically count
		this.UploadCounter = document.getElementById('UploadCounter');
	...
	onOpen: function(file, overall) {
		...
		this.currentTitle.set('html', this.options.fileProgress + ' "{name}"'.substitute(file) );
	},

	onProgress: function(file, current, overall) {
		this.overallProgress.start(overall.bytesLoaded, overall.bytesTotal);
		this.currentText.set('html', (this.options.uploadWith + ' {rate}/s. ' + this.options.timeLeft + ': ~{timeLeft}').substitute({
			rate: (current.rate && parseFloat(current.rate && current.rate != "Infinity") > 0) ? this.sizeToKB(current.rate) : '- B',
	...
	onComplete: function(file, response) {
		...
		this.currentText.set('html', this.options.uploadComplete + '!');
		this.currentProgress.start(100);
		(this.options.fileComplete || this.fileComplete).call(this, this.finishFile(file), response);
		// ExiteCMS MOD: update the upload counter
		if (this.UploadCounter) {
			this.UploadCounter.innerHTML = parseInt($('UploadCounter').get('html')) + 1;
		}
	},
	...
	updateOverall: function(bytesTotal) {
		...
		this.overallTitle.set('html', this.options.overallProgress + ' (' + this.sizeToKB(bytesTotal) + ')');
	},
	...
	fileCreate: function(file) {
		...
		file.element = new Element('li', {'class': 'file'}).adopt(
			...
			new Element('a', {
				'class': 'file-remove',
				'href': '#',
				'html': this.options.removeButton,
				'events': {
					'click': function() {
