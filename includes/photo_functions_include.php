...
function createthumbnail($filetype, $origfile, $thumbfile, $new_w, $new_h) {
	...
	switch ($filetype) {
		...
		case  IMAGETYPE_JEPG:
			...
			break;
		case  IMAGETYPE_PNG:
			if (!$origimage = imagecreatefrompng($origfile)) return false;
			break;
		case  IMAGETYPE_BMP:
			if (!$origimage = imagecreatefrombmp($origfile)) return false;
			break;
		case  IMAGETYPE_PSD:
			if (!$origimage = imagecreatefrompsd($origfile)) return false;
			break;
		default:
			//unsupported type
			return false;
	...
	else { imagepng($thumbimage, $thumbfile); }
}

// imagecreatefrombmp, thanks to DHKold (admin@dhkold.com)
...
// imagecreatefrompsd, Tim de Koning (Kingsquare Information Services, 22 jan 2007)
function imagecreatefrompsd($fileName) {
	include PATH_INCLUDES."class.psdreader.php";
	$psdReader = new PhpPsdReader($fileName);
	if (isset($psdReader->infoArray['error'])) return false;
